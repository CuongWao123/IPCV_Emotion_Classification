...
def augment_images(
    images, labels, target_class=1, target_count=2000, emotion_labels=None
):
    """
    ...
    Args:
        ...
        target_class: class to augment (1 = disgust)
        target_count: target number of samples after augmentation
    """
    print(
        f"\n Augmenting class {emotion_labels[target_class]} (class {target_class})..."
    )
    ...
    print(f"   Need to generate: {target_count - current_count} samples")
    ...
    print(
        f"   Generating {samples_per_original} augmented versions per original image..."
    )

    for idx in tqdm(target_indices, desc=f"Augmenting {emotion_labels[target_class]}"):
        original_img = images[idx].astype(np.float32)  # Convert to float for operations
        ...
        for aug_idx in range(samples_per_original):
            ...
            if aug_type == 0:
                # Horizontal flip
                aug_img = np.fliplr(original_img)

            elif aug_type == 1:
                # Rotation (-15 to +15 degrees)
                angle = np.random.uniform(-15, 15)
                aug_img = rotate(original_img, angle, reshape=False, mode="nearest")
                aug_img = np.clip(aug_img, 0, 255)

            elif aug_type == 2:
                # Brightness adjustment (for uint8 range [0,255])
                brightness_factor = np.random.uniform(0.8, 1.2)
                aug_img = original_img * brightness_factor
                aug_img = np.clip(aug_img, 0, 255)

            elif aug_type == 3:
                # Add Gaussian noise (scaled for uint8 range)
                noise = np.random.normal(
                    0, 5, original_img.shape
                )  # std=5 for [0,255] range
                aug_img = original_img + noise
                aug_img = np.clip(aug_img, 0, 255)
    ...
    X_combined = np.concatenate([images, np.array(augmented_images)], axis=0)
    y_combined = np.concatenate([labels, np.array(augmented_labels)], axis=0)
    ...
    print(f"   Total samples: {len(images)} → {len(X_combined)}")
    print(
        f"   {emotion_labels[target_class]} samples: {current_count} → {np.sum(y_combined == target_class)}"
    )
...
class EmotionDataPreprocessor:
    ...
    def _apply_median_filter(self, images):
        ...
        return np.array(
            [cv2.medianBlur(img, 3) for img in tqdm(images, desc="Median Filter")]
        )
    ...
    def _resize_and_normalize(self, images):
        ...
        resized = np.array(
            [cv2.resize(img, self.target_size) for img in tqdm(images, desc="Resize")]
        )
        normalized = resized.astype("float32") / 255.0
        with_channel = normalized.reshape(
            -1, self.target_size[0], self.target_size[1], 1
        )
        return with_channel
    ...
    def _lbp_grid_hist(self, img):
        ...
        for i in range(gy):
            for j in range(gx):
                ...
                hist, _ = np.histogram(
                    cell, bins=np.arange(0, n_bins + 1), range=(0, n_bins)
                )
    ...
    def transform(self, X_raw):
        ...
        if not self.is_fitted:
            raise ValueError(
                "Preprocessor must be fitted first! Call fit_transform() on training data."
            )
